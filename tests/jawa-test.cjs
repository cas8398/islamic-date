...
// 1️⃣ Multi-Year New Year (1 Suro) Validation
console.log("1️⃣ Testing New Year (1 Suro) Shifts");
...
const futureTransitions = [
  ...
  { date: new Date(2029, 4, 15), expectedYear: 1963, label: "2029 1 Suro" }, // Fixes your previous error
...
futureTransitions.forEach((t) => {
  ...
  const isMatch = res.year === t.expectedYear;
  console.log(
    `   ${isMatch ? "✅" : "❌"} ${t.label.padEnd(
      15
    )}: ${t.date.toLocaleDateString("en-GB")} -> ${res.year} AJ (${
      res.monthName
    })`
  );
});

// 1️⃣ LOCALIZATION CHECK (ID vs JV)
console.log("1️⃣ Testing Localization (Indonesian 'a' vs Javanese 'o')");
...
console.log(`   Indonesian : ${idMonth}`); // Should be "Sura"
console.log(`   Javanese   : ${jvMonth}`); // Should be "Suro"
console.log(
  idMonth !== jvMonth ? "   ✅ Localization: OK" : "   ❌ Localization: FAILED"
);

// 2️⃣ NEPTU & WETON LOGIC
console.log("\n2️⃣ Testing Weton & Neptu (Numerology)");
...
console.log(`   Total  : ${wetonTest.neptu.total}`); // Rabu (7) + Legi (5) = 12
console.log(`   Meaning: ${wetonTest.neptu.meaning}`);

// 3️⃣ YEAR TRANSITION (The 11-day Lunar Shift)
console.log("\n3️⃣ Testing Year Transition (1959 AJ -> 1960 AJ)");
...
transitionDates.forEach((t) => {
  ...
  const status = res.year === t.expected ? "✅" : "❌";
  console.log(
    `   ${status} ${t.name}: ${res.day} ${res.monthName} ${res.year} AJ`
  );
});

// 4️⃣ THE 5-DAY PASARAN CYCLE VALIDATION
console.log("\n4️⃣ Validating Pasaran (Multi-Boundary Check)");

const cycle = ["Legi", "Pahing", "Pon", "Wage", "Kliwon"];
const boundaryTests = [
...
let allPasaranPassed = true;

boundaryTests.forEach((t) => {
  const res = getJawaDate(t.date, "id");
  const passed = res.pasaran === t.expected;
  if (!passed) allPasaranPassed = false;

  console.log(
    `   ${passed ? "✅" : "❌"} ${t.date
      .toDateString()
      .padEnd(15)} -> ${res.pasaran.padEnd(7)} (Expected: ${t.expected})`
  );
});

console.log(
  allPasaranPassed
    ? "   ✅ All Pasaran Boundaries: OK"
    : "   ❌ Pasaran Drift Detected!"
);

// 5️⃣ FULL OUTPUT PREVIEW (For README/Docs)
console.log("\n5️⃣ Combined Calendar Output Preview");
...
console.log(`   🎋 Jawa   : ${full.jawaFormatted}`); // Format: Rabu Legi, 24 Rejeb 1959 AJ
console.log("   " + "═".repeat(40));
